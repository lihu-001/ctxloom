...
| `6-retrospective`         | 把坑与约定直接写回规则库（带背景注），让项目越用越聪明      |


> **bugfix 的根因定位手法**综合自 David Agans《Debugging》九大规则（看实际而非脑补、二分缩小范围、一次只改一处、先查最基础的假设等）与 Andreas Zeller《Why Programs Fail》的 Delta Debugging（系统化二分削减出最小复现）。它要求先用证据坐实根因、再动手改代码，避免"看起来像是这里"的臆测式修复。
...
### 可选但强烈建议：装上 CodeGraph，让 AI 真正读懂你的代码

ctxloom 的探索类技能（`init`、需求/设计/执行、`bugfix`）在理解现有代码时会**优先走结构化检索**——查符号定义、调用链、改动影响面，都比逐文件 `grep`+`read` 更准、更省上下文。这套能力由 [CodeGraph](https://github.com/colbymchenry/codegraph) 提供：它把整个代码库解析成符号与调用关系的知识图谱，作为 MCP server 暴露给 Claude Code。没装也能用，但装了之后 ctxloom 对中大型项目的理解会明显更靠谱。
...
之后 ctxloom 会在 CodeGraph 可用时自动改走结构化检索，无需额外配置；未安装时则自动退回 `grep`/读文件的方式，功能不受影响。
