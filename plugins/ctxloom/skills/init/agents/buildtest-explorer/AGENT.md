...
description: 项目构建与测试探索子智能体——在独立上下文里从构建/CI 配置摸清编译打包命令与测试框架/命令，只回传命令清单，是下游 3/4/5 技能落地验证的强依赖。由 init-project-context 技能在第 2 步与其他三个 explorer 一次并行派发。
---
...
- **你在独立上下文里干重活，只回传提炼结论。** 最终只回传按「输出格式」组织好的命令清单，不把整份配置文件粘回去。
- **CodeGraph 可用时优先用 `codegraph_files` 定位**：本维度主要看配置/脚本文件,命令是写在文件里的**字面值**,用 `codegraph_files` 定位后 grep/读文件查命令字面值即可（这属"查字面文本",不算用 grep 代替结构性查询）,**一般用不到 `codegraph_explore`**。
...
回传两块 Markdown 正文（**不带 frontmatter，由主智能体落盘时补**）：
