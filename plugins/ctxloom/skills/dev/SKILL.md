---
name: dev
description: 功能开发的统一入口——默认让任务走完整流程（需求→设计→计划→执行→提交→复盘），每一步按任务复杂度自适应缩放深度；只有极琐碎的改动才走逃生口直接做。触发：当用户希望让代码发生功能性变更（新增、修改、扩展某个功能或能力）时触发本技能。边界——① 修复已有缺陷走 bugfix，不走这里；② 用户明确表示"只想理清/分析需求、暂不写代码"走 1-requirements-analysis；③ 纯解释代码、答疑、不产生改动的问题不触发。意图不明时（如用户只抛出一个业务名词、不带任何动词，"订单导出""用户积分"），仍应召回本技能，但不要默认开干——进入后先一句话确认是"直接开发"还是"先理清需求"。
---
...
- **不存在** → **立即停止**，告知用户："检测到项目尚未初始化上下文目录（`.ctxloom/`），请先运行 `/init-project-context` 初始化，再回来继续开发。"等待用户操作，本技能退出。
  - 用户明确说"不需要初始化，直接开发"时，可继续，但必须警告：完整流程（标准档/深度档）强烈依赖上下文目录，建议仅在极简档逃生口下跳过。
...
档位定了，把它记到该功能的任务活文档（`.ctxloom/tasks/<功能>.md` 的 frontmatter 或开头，如 `grade: standard|deep`），作为**下游各步的默认起点**。然后依次走完整流程：
...
极简档逃生口里，`dev` 自己内联探索代码时，凡需要理解现有代码（定位类/接口/方法、摸清调用关系与改动波及面），**优先用结构化检索，而不是 `grep`/`find`/逐文件 `cat`**：
...
**任意档完成后，必须调用 `6-retrospective`。** 由它判断本次有无值得沉淀的经验——有就记，没有就直接结束，不强凑。标准档/深度档已将其纳入完整流程的最后一步；极简档逃生口在交付后立即调用。
